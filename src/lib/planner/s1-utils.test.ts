...
describe('computeEffectiveS1', () => {
  ...
  it('returns all visible when <= 3 candidates (no choice needed)', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi']
    expect(computeEffectiveS1(undefined, ['gat_passive_attr_str'], visible)).toEqual(visible)
  })

  it('returns all visible even when stored override exists', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd']
    const stored = ['gat_passive_attr_str']
    expect(computeEffectiveS1(stored, ['gat_passive_attr_str'], visible)).toEqual(visible)
  })

  it('returns empty when no visible candidates', () => {
    expect(computeEffectiveS1(undefined, ['gat_passive_attr_str'], [])).toEqual([])
  })
  ...
  it('uses solver selection filtered to visible when no stored override', () => {
    ...
    const solverS1 = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd']
    expect(computeEffectiveS1(undefined, solverS1, visible)).toEqual(solverS1)
  })

  it('filters solver selection to only visible stats', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd', 'gat_passive_attr_will']
    // solver selected 主能力提升 which is not visible
    const solverS1 = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_main']
    const result = computeEffectiveS1(undefined, solverS1, visible)
    expect(result).toEqual(['gat_passive_attr_str', 'gat_passive_attr_agi'])
  })

  it('falls back to visible when solver selection has no visible overlap', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd', 'gat_passive_attr_will']
    const solverS1 = ['gat_passive_attr_main'] // not in visible
  ...
  it('filters stored override to visible candidates', () => {
    const visible = [...ALL_S1]
    const stored = ['gat_passive_attr_str', 'gat_passive_attr_wisd', 'gat_passive_attr_will']
    expect(computeEffectiveS1(stored, ['gat_passive_attr_str'], visible)).toEqual(stored)
  })

  it('excludes stored stats that are no longer visible', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd', 'gat_passive_attr_will']
    const stored = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_main'] // 主能力提升 hidden
    const result = computeEffectiveS1(stored, ['gat_passive_attr_str'], visible)
    expect(result).toEqual(['gat_passive_attr_str', 'gat_passive_attr_agi'])
  })

  it('falls back to solver selection when stored has no visible overlap', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd', 'gat_passive_attr_will']
    const stored = ['gat_passive_attr_main'] // not in visible
    const solverS1 = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd']
  ...
  it('falls back to all visible when both stored and solver have no overlap', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd', 'gat_passive_attr_will']
    const stored = ['gat_passive_attr_main']
    const solverS1 = ['gat_passive_attr_main']
  ...
  it('auto-selects all visible when hiding reduces 5 candidates to 3', () => {
    const visible = ['gat_passive_attr_str', 'gat_passive_attr_agi', 'gat_passive_attr_wisd']
    const stored = ['gat_passive_attr_str', 'gat_passive_attr_wisd', 'gat_passive_attr_will'] // 意志提升 was valid before
