...
export interface FeedbackChannel {
  ...
  labelZh: string
  labelEn: string
  /** Locale-specific labels keyed by the four locale routes (used by buildGuardFeedbackHtml). */
...
export const FEEDBACK_CHANNELS = {
  github: {
    ...
    labelZh: 'GitHub',
    labelEn: 'GitHub',
    labels: { 'zh-CN': 'GitHub', 'zh-TW': 'GitHub', ja: 'GitHub', en: 'GitHub' },
  },
  forum: {
    href: 'https://end.302200.xyz',
    labelZh: '\u8BBA\u575B',
    labelEn: 'Forum',
    labels: { 'zh-CN': '\u8BBA\u575B', 'zh-TW': '\u8AD6\u58C7', ja: '\u30D5\u30A9\u30FC\u30E9\u30E0', en: 'Forum' },
  },
  qqGroup: {
    href: 'https://qm.qq.com/q/Cjdo2aRikE',
    labelZh: 'QQ\u7FA4 1045523485',
    labelEn: 'QQ Group 1045523485',
    labels: {
...
export function buildGuardFeedbackHtml(locale: string): string {
  ...
  const links = Object.values(FEEDBACK_CHANNELS).map((ch) => {
    const label = (ch.labels as Record<string, string>)[lang] ?? ch.labelEn
  ...
  return '<p style="font-size:12px;color:#999;margin:0;line-height:1.8;">'+
    FEEDBACK_TITLES[lang]+'<br>'+links+'</p>'
}
