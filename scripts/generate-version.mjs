 ...
 * 公告 markdown 的内容哈希清单。
 *
 * 生产环境 /announcements/*.md 返回 `max-age=31536000, immutable`, 但文件名不含
 * 内容指纹, 因此内容更新永远到不了老用户 —— 客户端此前只能用 `?t=Date.now()`
 * 破缓存, 结果是 immutable 完全失效、每次导航都重新下载全部 md。
 * 有了这份清单, 客户端改用 `?v=<hash>`, 可以真正吃到长缓存, 而更新时 hash 变化
 * 自然失效。index.generated.json 是发现新公告的唯一入口, 必须保持短缓存重验证,
 * 因此不纳入清单。
 */
export function generateAnnouncementManifest() {
  return generateHashManifest(
    resolve(root, "public", "announcements"),
    "/announcements",
...
function main() {
  ...
  const announcementManifest = generateAnnouncementManifest()
  const version = {
    commit: commitHash,
    count,
...
`)
  ...
  const announcementManifestPath = resolve(generatedDir, "announcement-hash-manifest.ts")
  writeFileSync(announcementManifestPath, `// Auto-generated by scripts/generate-version.mjs
export const announcementHashManifest: Record<string, string> = ${JSON.stringify(announcementManifest, null, 2)}
`)
  ...
  console.log(`announcement-hash-manifest.ts written: ${Object.keys(announcementManifest).length} entries`)
}
