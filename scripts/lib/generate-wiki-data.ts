...
/** Read planner source ids from disk instead of the module cache —
 * sync-game-data.ts rewrites src/data/weapons.ts and src/data/equips.ts
 * earlier in the same process, and a stale import would silently drop
 * newly added entries from planner-previews.ts.
 */
function readWeaponIdsFromDisk(projectRoot: string): Set<string> {
  const content = readFileSync(join(projectRoot, 'src', 'data', 'weapons.ts'), 'utf8')
  return new Set([...content.matchAll(/\bid:\s*'([^']+)'/g)].map((match) => match[1]))
}

function readEquipIdsFromDisk(projectRoot: string): Set<string> {
  const content = readFileSync(join(projectRoot, 'src', 'data', 'equips.ts'), 'utf8')
  // RAW_EQUIPS entries carry a static equipId literal; replicate the runtime
  // uniqueId dedup (-1, -2, ...) in case a primary id repeats.
  const seen = new Map<string, number>()
  const ids = new Set<string>()
  for (const match of content.matchAll(/equipId:\s*'([^']+)'/g)) {
    const primary = match[1]
    const count = seen.get(primary) ?? 0
    seen.set(primary, count + 1)
    ids.add(count === 0 ? primary : `${primary}-${count}`)
  }
  return ids
}
...
function writePlannerPreviewFile(dataDir: string, itemData: ItemWikiData): string {
  ...
  const previews = buildPlannerWikiPreviews(
    itemData,
    readWeaponIdsFromDisk(projectRoot),
    readEquipIdsFromDisk(projectRoot)
  )
  writeFileSync(path, [
